...
- `hand update` refreshes the workspace's AGENTS.md and reports release notes after a successful binary replacement, via `internal/agentsmd.Refresh`. That function (and `hand init`, which calls it too) draws the generated/user-content boundary with `<!-- hand:generated:start/end -->` markers around the fixed Workflow/Rules template: only the marked span is replaced on refresh, everything outside it is untouched, and a workspace is detected by the presence of `data/dashboard.md`. `selfupdate.ExecutableOverride` is exported (not the usual unexported test seam) specifically so `cmd/update_test.go` can redirect `Apply`'s binary target without touching the real test binary.
