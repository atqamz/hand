...
- `skills/` holds the Hand-owned files `hand init` writes: `bootstrap.md` becomes a fleet's `AGENTS.md`, and the `secondhand` and `secondhand-migrate` skills are installed per fleet. The binary's name replaces every `` `hand `` in them, so write commands as `` `hand …` ``.
