...
- `hand update` refreshes the workspace's AGENTS.md and reports release notes after a successful binary replacement, via `internal/agentsmd.Refresh`. That function (and `hand init`, which calls it too) draws the generated/user-content boundary with `<!-- hand:generated:start/end -->` markers around the fixed Workflow/Rules template: only the marked span is replaced on refresh, everything outside it is untouched, and a workspace is detected by the presence of `data/dashboard.md`.
`Refresh` writes only when the merged content differs from what is on disk, so it returns `refreshed=false` and leaves the file's inode and mode alone for an already-current or marker-less AGENTS.md; `cmd/update.go` treats a `Refresh` error as a stderr warning rather than a nonzero exit, since the binary is already replaced by that point. `selfupdate.ExecutableOverride` is exported (not the usual unexported test seam) specifically so `cmd/update_test.go` can redirect `Apply`'s binary target without touching the real test binary.
- Every write-to-temp-then-rename goes through `internal/atomicfile.Write(path, tempPrefix, data, mode)`; `dashboard`, `project`, `state`, `watcher`, and `agentsmd` all call it and wrap its error with their own context. Do not hand-roll another copy.
