...
if ($Check) {
    ...
    $result = Invoke-HandCapture $handPath $Fleet @('doctor')
    Write-BootstrapLog $result.Output
    if ($result.Code -ne 0) {
        Fail "hand doctor failed while checking $Fleet"
    }
    if ((Get-Field $result.Output 'ready') -cne 'true') {
...
Write-BootstrapLog $result.Output
if ($result.Code -ne 0) {
    Fail ("hand doctor failed for {0}; rerun HAND_HOME='{1}' {2} doctor after recovery" -f $Fleet, $Fleet, $handPath)
}
if ((Get-Field $result.Output 'ready') -cne 'true') {
