...
if [ "$check_only" -eq 1 ]; then
  ...
  if ! doctor_out=$(HAND_HOME="$fleet" "$hand_command" doctor 2>&1); then
    log "$doctor_out"
    die "hand doctor failed while checking $fleet"
  fi
  log "$doctor_out"
  [ "$(output_field ready "$doctor_out")" = true ] ||
...
if ! doctor_out=$(HAND_HOME="$fleet" "$hand_command" doctor 2>&1); then
  log "$doctor_out"
  die "hand doctor failed for $fleet; rerun HAND_HOME=$fleet $hand_command doctor after recovery"
fi
log "$doctor_out"
[ "$(output_field ready "$doctor_out")" = true ] ||
