...
func TestCanonicalDecisionCLI(t *testing.T) {
	...
	for _, id := range []string{"d_answer", "d_stale", "d_race"} {
		ok(question(id)...)
	}
	...
	for _, args := range [][]string{question("d_worker"), answer("d_answer", "a_worker", "option A")} {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM decision_answer`).Scan(&answers); err != nil || answers != 2 {
	...
	if err := db.QueryRow(`SELECT (SELECT COUNT(*) FROM worker_input)+(SELECT COUNT(*) FROM worker_wake_operation)+
		...
		(SELECT COUNT(*) FROM task_hold)+(SELECT COUNT(*) FROM external_operation)+(SELECT COUNT(*) FROM decision_closure)`).Scan(&invented); err != nil || invented != 0 {
		t.Fatalf("Decision interaction invented delivery/ack/effects/closure: %d %v", invented, err)
	}
}
