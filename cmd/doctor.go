...
func doctorLeaseHolderFindings(fleetHome string, projects []project.Project) []doctorFinding {
	...
	case errors.Is(err, registry.ErrRegistryMissing):
		// No Fleet has ever registered on this machine: nothing to look up, every holder below
		// classifies absent rather than unreadable.
	case err != nil:
	...
	for _, p := range projects {
		...
		for _, entry := range entries {
			...
			switch state, known := states[fleetID]; {
			case !known:
				findings = append(findings, doctorFinding{Severity: doctorError, Text: fmt.Sprintf("pool slot %s is leased to task %q of Fleet %s, which is absent from the Fleet registry; the lease is not reclaimed automatically, run `hand fleet` and decide with the operator", entry.Path, taskID, fleetID)})
			case state != registry.StateReady:
				findings = append(findings, doctorFinding{Severity: doctorWarning, Text: fmt.Sprintf("pool slot %s is leased to task %q of Fleet %s, which is registered but not ready (%s); the lease is not reclaimed automatically, run `hand fleet` to see why", entry.Path, taskID, fleetID, state)})
			}
		}
	}
	return findings
