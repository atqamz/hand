...
func TestAdoptRefusesDifferentDirectAndPathAuthoritiesBeforeMutation(t *testing.T) {
	want := BuildInfo{Version: "1.2.3", Channel: ChannelStable, Commit: stableTestCommit, Distribution: DistributionGitHub}
	source := writeIdentityExecutable(t, want, "source")
	target := testHandPath(t.TempDir())
	writeIdentityExecutableAt(t, target, BuildInfo{Version: "1.0.0", Channel: ChannelStable, Commit: stableTestCommit, Distribution: DistributionGitHub}, "direct")
	pathDir := t.TempDir()
	pathHand := testHandPath(pathDir)
	writeIdentityExecutableAt(t, pathHand, BuildInfo{Version: "1.0.0", Channel: ChannelStable, Commit: stableTestCommit, Distribution: DistributionBrew}, "package")
	...
	pathBefore, err := os.ReadFile(pathHand)
	if err != nil {
		t.Fatal(err)
	}
	_, err = Adopt(context.Background(), source, target, want)
	if err == nil || !strings.Contains(err.Error(), "different authorities") {
		t.Fatalf("Adopt() error = %v, want a different-authority refusal", err)
	}
	targetAfter, err := os.ReadFile(target)
	if err != nil {
		t.Fatal(err)
	}
	pathAfter, err := os.ReadFile(pathHand)
	if err != nil {
		t.Fatal(err)
	}
	if string(targetAfter) != string(targetBefore) || string(pathAfter) != string(pathBefore) {
		t.Fatal("different-authority refusal changed an executable")
	}
}

func TestAdoptReusesTheSameExecutableWhenPathNamesTheTarget(t *testing.T) {
	want := BuildInfo{Version: "1.2.3", Channel: ChannelStable, Commit: stableTestCommit, Distribution: DistributionGitHub}
	source := writeIdentityExecutable(t, want, "source")
	targetDir := t.TempDir()
	...
	if got.Path != target || got.Result != "reused" {
		t.Fatalf("Adopt() = %#v, want same-path reuse", got)
	}
}
