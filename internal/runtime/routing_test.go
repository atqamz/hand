...
func TestReopenSchemaV10MigratesOnlyAfterValidRouting(t *testing.T) {
	...
	if got := runtimeStoreSchemaVersion(t, home); got != 13 {
		t.Fatalf("schema version after valid Reopen = %d, want 13", got)
	}
}
...
func TestPromoteSchemaV10MigratesOnlyAfterValidRouting(t *testing.T) {
	...
	if got := runtimeStoreSchemaVersion(t, home); got != 13 {
		t.Fatalf("schema version after valid Promote = %d, want 13", got)
	}
}
