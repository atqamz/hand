...
`pane run` types its command line into the pane, and the pane shell runs it after the call returns; `Herdr.PaneRunShell` models that through `sh -c` without the pane terminal.
