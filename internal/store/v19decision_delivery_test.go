...
func TestCanonicalV19DecisionDeliveryRefusesSuccessorOfTriggeringReport(t *testing.T) {
	for _, scope := range []string{"task", "plan", "attempt"} {
		t.Run(scope, func(t *testing.T) {
			...
			if _, err := CreateCanonicalV19WorkerInput(nil, fixture.Home,
			...
			if _, err := CreateCanonicalV19AnswerWorkerInput(nil, fixture.Home, input); !errors.Is(err, ErrCanonicalV19WorkerInputConflict) {
...
func TestCanonicalV19DecisionDeliveryReplaysWithoutWakeOrAcknowledgement(t *testing.T) {
	...
	for _, scope := range []string{"task", "plan", "attempt"} {
		...
		first, err := CreateCanonicalV19AnswerWorkerInput(nil, fixture.Home, input)
		if err != nil {
			t.Fatal(err)
		}
		// A fresh writer/connection after input commit models retry before any wake.
		replayed, err := CreateCanonicalV19AnswerWorkerInput(nil, fixture.Home, input)
...
func canonicalV19DecisionTestStopExecutor(t *testing.T, home string, launch CanonicalV19LaunchRequest) {
	t.Helper()
	request, err := PrepareCanonicalV19Interrupt(nil, home, canonicalV19InterruptPrepareInput(launch, "interrupt-"+launch.BindingID))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := SubmitCanonicalV19Interrupt(nil, home, request.OperationID, "2026-09-21T10:00:00Z", "submitted"); err != nil {
		t.Fatal(err)
	}
	if err := CompleteCanonicalV19Interrupt(nil, home, CanonicalV19ExecutorInterruptedEvidence{
...
func canonicalV19DecisionTestRetryExecutor(t *testing.T, home, predecessorID string) CanonicalV19LaunchRequest {
	t.Helper()
	if err := TerminalizeCanonicalV19Attempt(nil, home, CanonicalV19AttemptTerminalizeInput{
		AttemptID: predecessorID, Lifecycle: "interrupted", TerminalAt: "2026-09-21T10:02:00Z",
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := CreateCanonicalV19Attempt(nil, home, canonicalV19AttemptWriterInput("attempt-2", "plan-root")); err != nil {
		t.Fatal(err)
	}
	create := canonicalV19WorktreeCreatePrepareInput(home, "worktree-create-2", "worktree-binding-2")
	create.AttemptID = "attempt-2"
	worktree, err := PrepareCanonicalV19WorktreeCreate(nil, home, create)
	if err != nil {
		t.Fatal(err)
	}
	if err := EstablishCanonicalV19WorktreeBinding(nil, home, canonicalV19WorktreeBindingEvidence(worktree, "physical-2")); err != nil {
		t.Fatal(err)
	}
	...
	session, err := PrepareCanonicalV19SessionAcquire(nil, home, acquire)
	if err != nil {
		t.Fatal(err)
	}
	if err := EstablishCanonicalV19SessionBinding(nil, home, CanonicalV19SessionBindingEvidence{
	...
	launch, err := PrepareCanonicalV19Launch(nil, home, canonicalV19LaunchPrepareInput(worktree, session, "launch-2", "executor-binding-2"))
	if err != nil {
		t.Fatal(err)
	}
	if err := EstablishCanonicalV19ExecutorBinding(nil, home, CanonicalV19ExecutorBindingEvidence{
