...
func readLegacyTask(path, id string) (Task, error) {
	...
	// A file predating the pane-start column carries no such key, and the import lands as an
	// INSERT the schema migration's backfill never sees. Same CASE as that backfill, so a
	// task promoted before either existed gets its own pane's start, not the #128 false one.
	if t.PaneStartedAt == "" {
