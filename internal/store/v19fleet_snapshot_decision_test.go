...
func TestReadCanonicalV19FleetSnapshotShowsOnlyCurrentOpenDecisions(t *testing.T) {
	...
	if err := CreateCanonicalV19Decision(ctx, home, closed); err != nil {
		t.Fatal(err)
	}
	...
	if err := CreateCanonicalV19Decision(ctx, home, openDecision); err != nil {
		t.Fatal(err)
	}
	...
	if err := CreateCanonicalV19Decision(ctx, home, other); err != nil {
		t.Fatal(err)
	}
	...
	if len(snapshot.CurrentOpenDecisions) != 2 {
		t.Fatalf("current open Decisions = %#v", snapshot.CurrentOpenDecisions)
	}
	first, second := snapshot.CurrentOpenDecisions[0], snapshot.CurrentOpenDecisions[1]
	if first.ID != openDecision.ID || first.TaskID != "task-1" || first.ScopeKind != "task" ||
		first.ChoicesDigest != openDecision.ChoicesDigest || first.CreatedAt != openDecision.CreatedAt ||
		second.ID != other.ID || second.TaskID != "task-2" {
		t.Fatalf("Decision snapshot lost exact rows or order: %#v", snapshot.CurrentOpenDecisions)
	}
	...
	if err != nil || len(retired.CurrentOpenDecisions) != 2 {
...
func TestCanonicalV19SnapshotCurrentDecisionsQueryUsesTaskHistoryIndex(t *testing.T) {
	...
	if !strings.Contains(plan.String(), "SEARCH d USING INDEX decision_task_history") ||
		strings.Contains(plan.String(), "SCAN d ") || strings.Contains(plan.String(), "SCAN c ") {
