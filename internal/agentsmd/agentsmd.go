...
var supervisorInstructions = []string{
	...
	"Immediately before finalizing a mechanical brief, resolve the registered project's verified default branch in `<home>/projects/<project>`, investigate against that revision, and record its full commit ID as `planned_against`. Dispatch compares the same local base later; if the project advances, re-check assumptions and rewrite or revalidate the plan rather than merely updating the SHA.",
