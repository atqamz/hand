...
func TestPayloadReferenceCloseRetainsManagedChildOwnership(t *testing.T) {
	store, path := installExecutableReferenceFixture(t)
	request := PayloadReferenceRequest{
		ReferenceID: "managed-child", LockScope: payloadReferenceLockScope("github/gh"), FleetID: integrationTestFleetID,
		Consumer: "integration-process", Evidence: "capability=github/gh",
	}
	reference, err := store.AcquireReference("github/gh", path, request)
	if err != nil {
	...
	if _, err := os.Stat(reference.RecordPath()); err != nil {
		t.Fatalf("child-owned payload record missing after parent close: %v", err)
	}
	if _, err := store.AcquireReference("github/gh", path, request); !errors.Is(err, ErrPayloadReferenceHeld) {
		t.Fatalf("acquire while managed child is alive = %v, want ErrPayloadReferenceHeld", err)
	}
	if err := os.WriteFile(release, []byte("release"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(reference.RecordPath()); err != nil {
		t.Fatalf("scoped child record missing after child exit: %v", err)
	}
	next := request
	next.ReferenceID = "next-child"
	recovered, err := store.AcquireReference("github/gh", path, next)
	if err != nil {
		t.Fatalf("next scoped reference after child exit: %v", err)
	}
	if err := recovered.Close(); err != nil {
		t.Fatal(err)
	}
