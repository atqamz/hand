...
func TestPaneRunCanonicalSpecRefusesNonIdleForegroundProcess(t *testing.T) {
	writeFakeHerdr(t, herdrResponse("pane process-info", `{"id":"cli:pane:process-info","result":{"type":"process_info","process_info":{"pane_id":"wA:p1","shell_pid":11,"foreground_process_group_id":22,"foreground_processes":[{"pid":11,"name":"bash","cwd":"/tmp/worktree"},{"pid":22,"name":"worker","cwd":"/tmp/worktree"}]}}}`))
	client := NewClient()
...
func TestPaneRunCanonicalSpecRefusesShellCwdMismatchBeforeMutation(t *testing.T) {
	writeFakeHerdr(t, herdrResponse("pane process-info", `{"id":"cli:pane:process-info","result":{"type":"process_info","process_info":{"pane_id":"wA:p1","shell_pid":11,"foreground_process_group_id":11,"foreground_processes":[{"pid":11,"name":"bash","cwd":"/tmp/other"}]}}}`))
	client := NewClient()
	err := client.PaneRunCanonicalSpec("wA:p1", launch.LaunchSpec{Executable: "worker", Cwd: "/tmp/worktree"})
