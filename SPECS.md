...
1. Arm: probe every active task's herdr pane once, before anything else. Any task that fails names it on stderr and exits `5` - never enters the baseline half-armed, since a task invisible to the very first probe has no herdr transition to ever fire on, and the wait would block until `--timeout` for a reason the caller never sees. Full per-tick probe-failure tracking and retry policy during the live poll that follows is a separate concern - see atqamz/secondhand#81.
