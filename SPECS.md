...
4. Classify each state change:
   ...
   - `pr-not-recorded <id>: <url> (<reason>)`: a PR URL a worker embedded in a report line was attempted and refused - it failed the validation `hand pr` enforces. The fix is a human running `hand pr <id> <url>`.
