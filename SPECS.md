...
`hand init` seeds `operator.md`, `learnings.md`, `done-archive.md` and `note-archive.md` once and no command writes them again (atqamz/secondhand#47); `hand update` seeds whichever of them the home is missing, so a home refreshed to a template that names them is never pointed at absent files (see "Self-update: `hand update`").
