...
4. On `--timeout`, write nothing to stdout, name the elapsed timeout on stderr, and exit `4`.
...
- **Arming itself can fail loudly, distinct from every other exit.** A worker that can't be probed at arm time is not a quiet fleet (`4`) and not a delivered event (`0`) - it is `5`, naming the worker, because the caller would otherwise wait out the full `--timeout` for a cause it can never see on stdout.
- **The exit code says which happened**: `0` an event was delivered, `4` no event (timeout or signal), `5` a task couldn't be probed at arm time, `1` the watcher itself failed, `2` a usage error. A caller can never read a crash or a quiet window as fleet news.
...
| The same bookkeeping across `hand promote` | Not covered by the identity check above: promote rewrites the task JSON in place and keeps `created_at`. `report_offset` is carried - promote never touches `state/<id>.status`, so the report stream is continuous and the offset already points exactly where the ship's first line lands; resetting it would replay the scout's consumed lines, the hazard the durable offset exists to prevent. `done_verified` is reset - the marker belongs to the scout's own verified `done`, not the ship's, which has not earned it yet; carrying it would leave the ship run unable to ever announce its own, since the write-back only ORs the marker to true. `status_changed_at` is restamped to the promotion time for the same reason one layer over: the scout's last observed transition happened in a pane the task no longer has, so it is not evidence about the ship's dwell, and carrying it would hand the ship a dwell already grown past `stale`'s threshold before its worker had run for a second. Neither reset is enough on its own: a long-running watcher's in-memory `TaskState` for the task survives the same identity check untouched, and would write its cached copy straight back onto the freshly-rewritten JSON on the very next tick. So `tick` forgets both. It forgets the cached `done_verified` whenever the freshly read task's has gone false while the tracked copy is still true - the only way that regression happens outside the watcher itself. It forgets the cached dwell whenever the freshly read `status_changed_at` differs from the value this watcher last persisted, reseeding it from disk exactly as a cold start would. That trigger is deliberately the disk value, not a newly observed status: a ship whose first probe happens to read the status the scout last held raises no transition to reseed the clock, and is precisely the case a status-conditioned rule would miss. |
...
6. Rewrite `state/<id>.json` in place: `kind` changes from `scout` to `ship`, and `harness`, `model`, `effort`, `worktree` and the `herdr` coordinates describe the new worker. `done_verified` is reset to false and `status_changed_at` is restamped to the promotion time - neither the scout's verified `done` nor its last observed transition carries to the ship. Every other field is carried, including `created_at` and the watcher's `report_offset` - see "What survives a `hand watch` restart", which classifies each of them.
...
- `5`: arm-time probe failure, only from `hand watch --until-event`: a task's herdr pane could not be probed before the wait began, named on stderr. Distinct from `4` because arming itself never completed, and from `0` because nothing was delivered (see "Delivering an event to a supervisory agent").
