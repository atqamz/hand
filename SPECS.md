...
4. Classify each state change:
   ...
   - `pr-not-recorded <id>: <url> (<reason>)`: a PR URL a worker embedded in a report line was not recorded - it failed the validation `hand pr` enforces, or the task was locked by another command at that moment. The report line is consumed either way, so this is a durable event on stdout and in `state/events.log` (plus a stderr diagnostic) rather than only a transient stderr line. On a validation refusal the fix is a human running `hand pr <id> <url>`; on lock contention the outcome is genuinely unknown, so the event says only that and points at `hand status <id>` to confirm. Nothing is announced when the lock holder is found to have already recorded that same URL. It is deliberately not a Pending Decision - see "Pending Decisions".
