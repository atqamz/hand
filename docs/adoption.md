...
If no supported Supervisor Harness is installed, `hand doctor` reports the missing readiness condition instead of guessing one.
...
Supervisor Harnesses are reported purely as installed or not - `hand` never picks one on the operator's behalf. Worker Harness routing is a separate configuration surface, so a worker-only provider such as Antigravity cannot make bootstrap claim Supervisor readiness.
