...
Binds `hand watch`, `hand spawn`, `hand promote`, and `hand reconcile`, through [`internal/watcher/usagelimit.go`](../../internal/watcher/usagelimit.go), [`internal/runtime/launch.go`](../../internal/runtime/launch.go), and [`internal/runtime/liveness.go`](../../internal/runtime/liveness.go), the only three callers of `herdr.Client.PaneRead`.
...
Usage-limit detection is the one place a scrape reaches durable state today, as a `limit` hold and an attempt's usage-limit episode, and it is inside this invariant rather than an exception to it: the hold is a schedule the mechanism clears itself, not a claim about the task. `state.Hold.Inferred` is the representation that carries this: `hand watch` and `hand reconcile` both set it on the limit hold they write, and `hand status`/`hand hold`'s renders append it wherever the hold's detail appears.
