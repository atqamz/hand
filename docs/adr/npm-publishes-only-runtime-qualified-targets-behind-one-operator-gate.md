...
Referencing an `environment:` name that does not yet exist as a repository environment does not fail and does not wait: GitHub auto-creates it with no protection rules at all, and the job runs immediately, unattended. The gate is real only once the environment exists with a required reviewer configured - see [`docs/npm-trusted-publisher-enrollment.md`](../npm-trusted-publisher-enrollment.md)'s prerequisite section for the exact state and how to verify it. As of 2026-08-29 the `npm-publish` environment exists on `atqamz/hand` with `atqamz` as its required reviewer and a protected-branches deployment policy (`main` is protected, so this is satisfiable), verified via `gh api repos/atqamz/hand/environments/npm-publish`.
...
Platform packages publish before the meta package, in the order `generate.sh` derives, because the meta package's `optionalDependencies` pin platform-package versions that must already exist for `npm install` to resolve. The GitHub release's `publish` job is deliberately independent of `npm-publish`: GitHub releases are the primary distribution channel and npm is additional, so a pending approval, a rejected gate, or an npm outage must never hold back the primary one. `publish` completes from its own `build` artifacts alone, regardless of `npm-publish`'s outcome.
...
- Making the GitHub release's `publish` job depend on `npm-publish` (so a pending or failed npm gate would also hold the GitHub release) was tried and reverted: it inverts channel priority, since GitHub releases are the primary distribution channel and npm is additional - an npm outage or a rejected approval must never block the primary channel.
