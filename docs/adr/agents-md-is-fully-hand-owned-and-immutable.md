...
A fleet home's `AGENTS.md` can no longer coexist with hand-authored operator prose; anything an operator wants to keep must live under `data/**` or in the bundled skill's references instead. The one-time migration is a single archive-and-replace rather than an ongoing merge, so `internal/agentsmd` sheds the perishable-content scanner, the marker-drift line-by-line diagnostics, and `mergeGenerated` entirely. This repository's own top-level `AGENTS.md` is exact canonical content, dogfooded by `internal/agentsmd`'s exact-match test; no runtime code reads legacy markers there.
