...
An idle-unreported Attempt whose harness carries a catalogued usage-limit signature ([usage-limit detection is a harness capability](usage-limit-detection-is-a-harness-capability.md)) is additionally probed once for that signature. A detected limit is recorded the same way the watcher's own detection would: `usage_limit_retry_at`/`usage_limit_episode` durably updated and a `HoldKindLimit` hold projected, preserving the harness's own stated reset time when it supplies one and inventing none when it does not. Reconcile only detects; it never steers a pane, which stays exclusively the watcher's concern, and it never re-probes an attempt whose retry schedule is already durable, leaving that backoff to whichever mechanism is already managing it.
