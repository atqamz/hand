...
source_body_updated_at: 2026-09-14T13:27:46Z
contract_version: v19-read-model-v3
supersedes_revision: b2e11b5e14007eb46bdc1e820d4e085afd4caa57
supersedes_path: docs/architecture/v19-contracts/347-read-models-attention-orientation-v2.md
supersedes_blob: bf793d5e3dca230f15e808f5e516db5a0c7cbcc3
---

# Canonical read model, WorkerReport source proof, Task archive, Attention, and SupervisorOrientation — revision 3

This repository snapshot is the revision-3 candidate that supersedes the revision-2 #347 snapshot when the permanent manifest anchor names this content. Every prior file remains immutable historical evidence. The GitHub issue is a tracker/discussion surface; comments are not normative architecture state.

Revision 3 relocks the exact WorkerReport relational tail required by PR #581. Revision 2's source-witness boundary and Task archive projection rules remain unchanged, as do every prior read-model, WorkerInput, Attention, and orientation rule.
...
For one exact Attempt, canonical source order is the strict order of immutable
`source_end_offset` values. The named unique index is:

```sql
CREATE UNIQUE INDEX worker_report_attempt_source_order
ON worker_report(attempt_id, source_end_offset);
```

Uniqueness rejects two different immutable prefixes at the same source boundary.
It does not claim that an offset proves historical bytes. Exact source continuity
still requires the predecessor's complete `(id, source_prefix_digest,
source_end_offset)` tuple from an attested append witness.

One bounded indexed query identifies the relational tail for an exact Attempt:

```sql
SELECT id, source_prefix_digest, source_end_offset
FROM worker_report
WHERE attempt_id = ?
ORDER BY source_end_offset DESC
LIMIT 1;
```

The WorkerReport writer must hold one `BEGIN IMMEDIATE` transaction while it
compares the append witness's exact predecessor tuple with that relational tail.
The first report requires both a nil predecessor and no relational tail. Every
later report requires an exact predecessor match and a strictly greater resulting
offset. An exact immutable replay converges without insertion. A same-offset
different-prefix conflict, a predecessor other than the relational tail, or stale
Attempt/ExecutorBinding evidence refuses the append. These rules make a restored
R1 checkpoint unable to fork after canonical R2, even if the proposed fork has a
distinct offset.

The disposable checkpoint is usable only when its exact tuple also equals the
relational tail. Missing, corrupt, uncertain, stale, or contradictory checkpoint
state uses full replay or refuses; syntactic validity alone never authorizes an
append. Full replay compares every ordered immutable report with the exact source
witness chain, rejects truncation or changed historical prefixes, and inserts
only a proven suffix.
...
## Positive bounded source witness

The same-user mutable `state/<id>.status` file cannot positively prove that a previously read historical prefix remains unchanged. An append after prefix `P` and an in-place rewrite to equal-length `P'` followed by the same suffix expose identical unread bytes. Portable file metadata proves neither history nor an unchanged prefix.

Therefore an ordinary mutable report file never qualifies by itself for bounded incremental checkpoint advancement. None of these alone proves the historical prefix:

```text
serialized rolling hash state
byte offset
...
A bounded incremental WorkerReport path requires an exact predecessor-linked append witness. Each witness binds the exact Attempt, exact predecessor witness, appended record bytes/digest and resulting boundary, and the exact ExecutorBinding when one exists. The chain establishes append continuity from a positively known predecessor; it does not merely summarize current bytes.

Changed, missing, contradictory, or unprovable predecessor/source history is contradiction or unknown. The ingester must refuse checkpoint advancement or use the full replay fallback. It never guesses continuity from an offset, equal length, inode, timestamp, event, or rolling state.

The append witness, relational tail, source-order index, offset, and checkpoint are mechanism evidence only. None is WorkerReport content/currentness, WorkerReport acknowledgement, WorkerInput acknowledgement, Decision/Answer authority, or progression authority. WorkerReport remains a Claim. The `worker_report` columns and the separate `worker_report_acknowledgement` relation remain unchanged; revision 3 adds only source-boundary uniqueness and the bounded source-order index.

This contract chooses properties, not a provider/runtime mechanism. Exact source production must coordinate with #346's observation boundary and #579's launch-channel delivery work. No ordinary mutable path is described as qualified, and this revision introduces no generic reporting service or persisted predecessor infrastructure.
...
# Task archive projection

Task archive is derived from the single exact `task_archive` fact owned by #344/#345. It is projection classification only.
...
12. exact Task archive membership and unresolved obligations for archived Tasks.
...
```text
ingest newly observable report material only through full replay or an exact predecessor-linked append witness
read canonical DB
...
- [ ] WorkerReport full replay remains the correctness fallback.
- [ ] Bounded incremental ingestion requires an exact predecessor-linked append witness bound to the exact Attempt and, when present, ExecutorBinding.
- [ ] Ordinary mutable files, rolling hash state, offsets, inode identity, timestamps, watcher events, and sampled bytes alone never prove an unchanged prefix.
- [ ] Missing, changed, contradictory, or unprovable source history refuses checkpoint advancement or uses full replay.
- [ ] One indexed relational-tail query returns the exact `(id, source_prefix_digest, source_end_offset)` tuple for one Attempt.
- [ ] Source boundaries are unique per Attempt; exact-tail comparison and strict append ordering reject restored-predecessor forks at equal or distinct offsets.
- [ ] Offset ordering never substitutes for exact prefix/predecessor proof.
- [ ] Append witness remains mechanism evidence; WorkerReport remains a Claim and acknowledgement authority stays relationally separate.
