# v19 semantic contract manifest — revision 9

This is the current repository-owned v19 semantic and relational contract anchor. It supersedes `manifest-v8.md` as current authority. Earlier manifests, candidate inputs, snapshots, and artifacts remain immutable historical evidence.

## Permanent anchor and immutable content

Permanent `main` revision containing the complete revision-9 content set:
...
The later commit that adds this manifest is not a replacement content revision. It also does not claim that the candidate `manifest-v9-input.md` was itself permanent authority.
...
The #346 revision-2 and #348 revision-4 snapshots supersede earlier revisions without editing historical bytes.

- **#346 revision 2** supersedes revision-1 blob `859b80207a625fb4be8f5ff1a5eaf336bb7e8c77`. It gives the `herdr` adapter a Hand-owned execution guard, which supplies:
  - execution identity;
  - executable-object identity;
  - positive cessation;
  - WorkerInput caller attestation.

  Linux and Windows are enabled per platform. macOS keeps the revision-1 refusals.
- **#348 revision 4** supersedes revision-3 blob `b67acfad620ca08ff031531ce93fe5e8e39589f9`. It makes cutover offline only. Before publication it requires a committed boot-identity witness and a post-witness drift gate.
- **`manifest-v9-input.md`** named #346 revision 1. Revision 2 landed in #664 before this anchor and replaces that row here. The input stays unedited historical candidate evidence.

This manifest adds no DDL, runtime behavior, dependency, or workflow.

- It does not implement or qualify the exec guard or the offline cutover.
- It does not complete #346, #348, or #305 exact-candidate release qualification.
