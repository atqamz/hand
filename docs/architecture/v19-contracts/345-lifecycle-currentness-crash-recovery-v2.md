...
source_body_updated_at: 2026-09-14T13:27:44Z
contract_version: v19-lifecycle-v2
supersedes_revision: 899e7a9aa2e383d3b3f91689fe5a897c5bab59ec
supersedes_path: docs/architecture/v19-contracts/345-lifecycle-currentness-crash-recovery.md
supersedes_blob: 1039bb9fe69fb84f4b2357c05e6dcbfc6dfb3254
---

# Lifecycle, transaction, currentness, concurrency, crash-recovery, and Task archive contract — revision 2

This repository snapshot is the revision-2 candidate that supersedes the original #345 snapshot when the permanent manifest anchor names this content. The original file remains immutable historical evidence. The GitHub issue is a tracker/discussion surface; comments are not normative architecture state.

Revision 2 adds only exact Task archive currentness, concurrency, and post-archive evidence rules for atqamz/hand#301. Every prior lifecycle, WorkerInput, effect, resource, and recovery rule remains unchanged.
...
| Task archive vs pre-existing obligation | obligation commits first, archive predicate fails with no archive fact |
| Task archive vs later evidence | archive commits first, later evidence remains visible without lifecycle reopen |
| duplicate/conflicting Task archive | exact replay converges on one fact; conflict refuses and cannot rewrite history |
...
# Task archive currentness

Task archive is one immutable projection-classification fact for one exact Task. It is not Task lifecycle, semantic completion, teardown, acknowledgement, delivery, qualification, integration, or cleanup.

The exact relation records only:

```text
exact task_id
bounded actor kind/reference
archived_at
bounded reason
exact evidence digest
```

There is no `Task.lifecycle=archived`, descendant archive flag, generalized archive subsystem, deletion, or unarchive in v0.8.

The archive writer uses one transaction:

```text
establish fresh positive external resource observations
BEGIN IMMEDIATE
revalidate exact Task identity and observation identities
...
A historical unacknowledged WorkerInput is not automatically a current obligation after its exact Attempt/Executor terminates. Archive does not fabricate WorkerInputAcknowledgement, WorkerReport acknowledgement, receipt, qualification, integration, cleanup, delivery, or success to make a predicate pass.

SQLite writer serialization defines the archive race. An obligation committed before archive is visible to the same `BEGIN IMMEDIATE` predicate and blocks the insert. If archive commits first, a later handling-worthy report, Decision, Repair, or other evidence may still append. That later evidence remains canonical and visible through #347 Attention/history without deleting the archive fact or reopening Task/Plan/Attempt lifecycle.

Duplicate exact replay converges on the single task-keyed fact. A conflicting actor/time/reason/evidence tuple refuses; no last-write-wins rewrite is permitted. A stale request always names and revalidates the exact Task and never retargets a successor.
...
- [ ] Task archive is one immutable exact-Task fact and never a lifecycle value, descendant flag, deletion, or unarchive mechanism.
- [ ] Archive re-proves the full relational predicate in one `BEGIN IMMEDIATE` transaction and requires positive external observations.
- [ ] A pre-existing obligation blocks archive; evidence appended after archive remains visible without lifecycle reopen.
- [ ] Duplicate archive replay converges and conflicting/stale archive requests refuse without retargeting.
