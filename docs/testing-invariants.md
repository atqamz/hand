...
|---|---|---|---|
| INV-TASK-1 | At most one attempt of a task is provisioning or running, at any point in any sequence of operations. | model | `TestTaskAttemptLifecycleStateMachine`, at the store layer: the partial unique index and every guarded UPDATE it drives directly. A runtime-level path that reached a second active attempt without going through a store guard would not be caught here - see `internal/runtime` callers of the same primitives. |
| INV-TASK-2 | Attempt ordinals within a task are contiguous and start at 1, and an ordinal is never reused. | model | `TestTaskAttemptLifecycleStateMachine`, at the store layer (see INV-TASK-1) |
| INV-TASK-3 | A terminal attempt is immutable with respect to activation: no operation reactivates it. | model | `TestTaskAttemptLifecycleStateMachine`, at the store layer (see INV-TASK-1) |
| INV-TASK-4 | The report cursor is continuous across attempts: promotion and reopen do not reset or rewind it. | model | `TestTaskAttemptLifecycleStateMachine`, at the store layer (see INV-TASK-1) |
| INV-TASK-5 | Teardown terminalizes the task and attempt rows and deletes neither. | property | phase 5 of atqamz/hand#442 covers this at the stronger model layer, `TestTaskAttemptLifecycleStateMachine`, at the store layer (see INV-TASK-1), as the teardown transition's natural companion; property form otherwise unaudited |
| INV-TASK-6 | `hand reopen` on a terminal task creates a new attempt and mutates no existing one. | model | `TestTaskAttemptLifecycleStateMachine`, at the store layer (see INV-TASK-1) |
| INV-TASK-7 | Promotion preserves the scout attempt and creates the next one. | model | `TestTaskAttemptLifecycleStateMachine`, at the store layer (see INV-TASK-1) |
| INV-TASK-8 | An existing attempt's execution identity - harness, model, effort, execution class, planned-against commit, requested profile, routing source - is write-once. Nothing reroutes it after creation. | model | `TestTaskAttemptLifecycleStateMachine`, at the store layer (see INV-TASK-1) |
...
| id | invariant | layer | coverage |
|---|---|---|---|
| INV-REC-1 | Reconciliation converges: applied twice to unchanged observed reality, the second run changes no durable state. | model | `TestReconciliationConvergesOnUnchangedObservedReality`, at the runtime layer: real `internal/state` store, faked Herdr/worktree observation deps, across 5 self-consistent (durable intent, observed reality) scenarios - each calls `Reconcile` twice against unchanged fakes and compares the durable snapshot byte-for-byte (JSON of `state.ReadHistory` plus `state.ReadHold`, not a chosen field subset), with a ticking clock so a missing idempotency guard would be caught rather than masked by a fixed timestamp |
| INV-REC-2 | The decision table is a function of (durable intent, observed evidence). Same pair, same decision, for any pair. | property | `TestDecideReconciliationIsAPureFunctionOfDurableIntentAndObservedEvidence`, at the pure-function layer: `decideReconciliation` takes no receiver and performs no I/O, so no store or fakes are needed - a generated (task, attempt, observation) triple is passed to it twice and the two decisions are required to be `==`-equal |
| INV-REC-3 | An observation *failure* never becomes contradiction evidence, and never clears an existing repair marker. | property | `TestObservationFailuresNeverBecomeContradictionEvidenceOrClearARepairMarker`, at the runtime layer: generates failures on each of the three external observation surfaces reconcile reads (Herdr, worktree/Treehouse, GitHub), distinguishing a hard I/O error (must abort before any decision is made) from an "unobservable" classification (must route to a repair code that says so, never a proven mismatch or absence, and never clear an existing repair marker) |
| INV-REC-4 | A repair marker survives until the same contradiction is proven gone or a safe lifecycle transition resolves it. | model | `TestRepairMarkerSurvivesAnOrthogonalChangeAndClearsWhenTheSameContradictionIsProvenGone`, at the runtime layer: two scenarios (Herdr identity axis, worktree/Treehouse lease axis) each driving contradiction-observed to repair-set, an orthogonal change on the other axis to repair-survives-unchanged, then the same axis proven healthy to repair-clears; the worktree scenario also exercises the Running-lifecycle carve-out in `shouldClearRepair`. The "safe lifecycle transition resolves it" clause is covered separately by the pre-existing `TestReconcileClearsRepairAfterTeardownAndReopenResolvesOldAttempt`, a full teardown/reopen worked example |
| INV-REC-5 | Reconciliation applies at most one action per loop iteration, then re-observes. | model | `TestReconcileAppliesExactlyOneActionPerIteration`, `TestReconcileTaskReObservesFreshStateEveryIteration`, at the runtime layer, against `reconcileTask`'s own loop: the first drives confirm-launch and mark-running (two distinct actions) and requires `Iterations` to count 3 rather than collapsing to 2; the second switches the observed pane identity partway through the same convergence and requires the final decision to reflect what the last iteration actually observed, not what the first one saw |
| INV-REC-6 | Automatic resource cleanup requires exact ownership proof, a clean worktree, and positive proof that returning the worktree discards no commit held nowhere else. All three, never two. | property | `TestAutomaticWorktreeReturnRequiresOwnershipCleanlinessAndCommitSafetyConjunctively`, at the runtime layer: sweeps all 5 lease states x 2 cleanliness states x 3 commit-safety states (30 combinations, including every 2-of-3 case) from the real fresh `teardown_worktree_state` a completed attempt starts with, and asserts the automatic worktree-return fake fires exactly in the all-three-good combination and never otherwise. The `"" -> released` regression atqamz/hand#482 found is additionally covered at the unit layer by `TestReconcileConvergesAWorktreeWhoseLeaseReadsAbsentBeforeTeardownStateIsEverSet`, `TestReconcileConvergesAnAmbiguousHerdrOnceOwnershipReadsAbsent`, `TestReconcileConvergesARetryableWorktreeOnceLeaseReadsExactAgain` |
| INV-REC-7 | Classifying an attempt idle-unreported changes no lifecycle, releases no lease, returns no worktree, and touches no Herdr resource. | property | `TestIdleUnreportedClassificationChangesNoLifecycleLeaseWorktreeOrHerdrResource`, at the runtime layer, calling `recordAttemptLiveness` directly (not through the full `Reconcile` loop) so the property isolates exactly this claim: no lifecycle write, no lease release, no worktree return, and none of Herdr's resource-owning calls fire (Workspace/Tab create or close, PaneRunSpec, PaneSendKeys, PaneRun - PaneRead is the documented fallback probe this mechanism is built on, not a resource touch, per the attention ADR's own vocabulary) |
...
| INV-REC-8 | The usage-limit probe fires at most once per stop. | model | `TestUsageLimitProbeFiresAtMostOncePerStop`, at the runtime layer: drives `recordAttemptLiveness` across a generated sequence of pane-status polls, re-reading the attempt from the store between calls the way `reconcileTask` itself does; a reference model independently tracks how many fresh idle/done stops the sequence contains and asserts the probe fires exactly once per fresh stop, never on a repeat poll of the same stop |
...
| INV-PROJ-2 | Rename writes exactly one row, found by id, and nothing else. | property | unit - `TestRenameRelabelsOneRowAndLeavesTaskHistoryAlone`; property - `TestRenameWritesExactlyOneRowFoundByIDAndNothingElse` |
| INV-PROJ-3 | A name is unique among registered projects and reusable after removal. | model | unaudited |
| INV-PROJ-4 | A completion record keeps the label it was written with; it is an audit line, not a view. | property | unit - `TestAppendListRoundTrip`; property - `TestCompletionRecordsRoundTripTheirWrittenLabelAcrossArbitraryAppends` |
| INV-PROJ-5 | The completion file is append-only. The single exception - the one-time project-identity migration (`completion.MigrateProjectIdentity`) replacing the whole file through a temp file and a rename under the same lock - never touches an unrelated record, including on rollback. Corrected from an earlier wording naming project rename as the exception: rename stopped touching completion records once records and tasks started referencing a project's surrogate id instead of its mutable name (atqamz/hand#388, atqamz/hand#396); `internal/project/project.go`'s `Rename` says so at its own call site. | property | unit - `TestMigrateProjectIdentityIsIdempotentAndLeavesDamagedLinesAlone`, `TestMigrateProjectIdentityWritesNothingWhenThereIsNothingToDo`; property - `TestMigrateProjectIdentityRewritesOnlyLegacyLinesAndNeverGuessesAnUnresolvedLineage` (rewrite half), `TestMigrateProjectIdentityLeavesTheStoreExactlyAsItWasWhenTheReplaceFails` (rollback half, unix-only - see the test's own comment) |
| INV-PROJ-6 | Unplaceable lineage is marked, never guessed: an unresolvable project id is written as the explicit unknown value rather than empty. | property | unit - `TestMigrateProjectIdentityNeverGuessesFromTheNameAlone`, `TestMigrateProjectIdentityMarksUnresolvableRecordsUnknown`; property - `TestMigrateProjectIdentityRewritesOnlyLegacyLinesAndNeverGuessesAnUnresolvedLineage` |
| INV-PROJ-7 | A completion is appended before its task and attempt rows are terminalized, never after. | model | unaudited at model layer - restated by phase 5 of atqamz/hand#442: [Tasks are durable and Attempts own execution](adr/tasks-are-durable-and-attempts-own-execution.md) (atqamz/hand#193) replaced task-row removal at teardown with terminalization, so "removed" was the wrong verb; "terminalized" is what phase 4 flagged this row to become. The ordering itself still holds - `internal/runtime/teardown.go`'s `appendCompletion` runs before `finishTeardown`'s `TerminalizeTaskAndAttempt` - and is pinned for one boundary by the existing unit test `TestTeardownFailureAfterCompletionAppendLeavesBoundaryObservable` (stops right after the append and shows the task still open with the record already written). Phase 5's state machine does not reach this row: it drives `internal/store`'s lifecycle primitives directly, one layer below the two-subsystem ordering (a JSONL append, then a DB terminalize) this row is actually about, which lives entirely in `internal/runtime/teardown.go`. |
| INV-PROJ-8 | A completion record whose encoded line meets or exceeds the scanner's byte limit fails loudly and specifically, not like any other read failure: the error names `bufio.ErrTooLong`, the store's path, and the failing line's 1-indexed position, so an operator can find and repair that one line by hand - never truncated, never silently skipped the way a malformed line is. The failure is total for `List`, not local to that line: a record written before it is also unreadable from that same call, unlike a damaged line, which `List` skips individually while returning everything else. Reachable today, not hypothetical: a worker's own free-text report note becomes a completion record's `Detail` verbatim when the last report before teardown is `failed:` (`internal/runtime/teardown.go`'s `completionFor`), and nothing caps a single report line's length between a worker's redirect and that assignment - `internal/state/report.go` reads a report file whole and splits on newline, no scanner or size bound involved. | unit | `TestSurfacesTheReadErrorForARecordAtTheScannerLimit`, `TestFindAttemptSurfacesTheReadErrorNamingTheRealFileLine`, `TestSurfacesTheReadErrorNamingTheRealFileLineAcrossOtherLinesBeforeIt`, `TestListLosesPrecedingRecordsWhenALaterLineExceedsTheScannerLimit`; `TestRoundTripsARecordOneByteUnderTheScannerLimit` confirms the boundary's other side |
...
| INV-HOLD-1 | A hold is a standalone row with no foreign key to a task, and survives the task's teardown when human-authored. | property | unit - `TestHoldSurvivesWithNoTaskRowBehindIt`; property - `TestHumanAuthoredHoldSurvivesAnyTaskLifecycleAroundIt` |
| INV-HOLD-2 | A machine-authored hold is cleared only after its kind is checked, never by kind-blind clearing. | property | unit - `TestConditionalMachineHoldCannotOverwriteOperatorHold`, `TestConditionalMachineClearCannotDeleteOperatorReplacement`; property - `TestConditionalHoldOperationsCheckKindBeforeActing` |
