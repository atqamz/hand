...
| INV-REC-6 | Automatic resource cleanup requires exact ownership proof, a clean worktree, and positive proof that returning the worktree discards no commit held nowhere else. All three, never two. | property | unaudited at the property layer; the `"" -> released` regression atqamz/hand#482 found (a terminal attempt whose `TeardownWorktreeState`/`TeardownHerdrState` never passed through `releasing` before the resource read absent or ownership was re-proven) is covered at the unit layer by `TestReconcileConvergesAWorktreeWhoseLeaseReadsAbsentBeforeTeardownStateIsEverSet`, `TestReconcileConvergesAnAmbiguousHerdrOnceOwnershipReadsAbsent`, `TestReconcileConvergesARetryableWorktreeOnceLeaseReadsExactAgain` |
