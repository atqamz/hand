...
|---|---|---|---|
| INV-TASK-1 | At most one attempt of a task is provisioning or running, at any point in any sequence of operations. | model | `TestTaskAttemptLifecycleStateMachine` |
| INV-TASK-2 | Attempt ordinals within a task are contiguous and start at 1, and an ordinal is never reused. | model | `TestTaskAttemptLifecycleStateMachine` |
| INV-TASK-3 | A terminal attempt is immutable with respect to activation: no operation reactivates it. | model | `TestTaskAttemptLifecycleStateMachine` |
| INV-TASK-4 | The report cursor is continuous across attempts: promotion and reopen do not reset or rewind it. | model | `TestTaskAttemptLifecycleStateMachine` |
| INV-TASK-5 | Teardown terminalizes the task and attempt rows and deletes neither. | property | phase 5 of atqamz/hand#442 covers this at the stronger model layer, `TestTaskAttemptLifecycleStateMachine`, as the teardown transition's natural companion; property form otherwise unaudited |
| INV-TASK-6 | `hand reopen` on a terminal task creates a new attempt and mutates no existing one. | model | `TestTaskAttemptLifecycleStateMachine` |
| INV-TASK-7 | Promotion preserves the scout attempt and creates the next one. | model | `TestTaskAttemptLifecycleStateMachine` |
| INV-TASK-8 | An existing attempt's execution identity - harness, model, effort, execution class, planned-against commit, requested profile, routing source - is write-once. Nothing reroutes it after creation. | model | `TestTaskAttemptLifecycleStateMachine` |
