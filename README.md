...
`hand init` is non-interactive. It creates the fleet structure, writes an `AGENTS.md` that teaches a supervising agent how to operate Secondhand, and installs a Claude Code `SessionStart` hook for ambient fleet context.
...
The session opens with the current fleet overview already in context. The generated `AGENTS.md` is the operating manual for the supervisor and can also be read by other agent environments.
...
Inspect and configure fleet defaults with:
...
Model and effort support depends on the harness. `hand config` reports which values are supported instead of silently storing settings that a harness cannot carry.

A task brief can also declare model and effort for that specific worker; explicit spawn or promote flags win over brief values.
...
See the [releases page](https://github.com/atqamz/secondhand/releases) for every asset.
...
`go install` produces a binary named `secondhand`, not `hand`, and does not embed release-version metadata. Prefer a release binary or Nix installation for the normal experience.
...
Other commands cover configuration, project sync and upstreams, holds, scout promotion, search, notifications, diagnostics, PR recording, and self-update.
